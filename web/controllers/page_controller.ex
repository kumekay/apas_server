defmodule ApasServer.PageController do
  ...
  def index(conn, _params) do
    ...
    pourings = from(p in ApasServer.Pouring, order_by: [desc: p.inserted_at], select: [p.volume, p.inserted_at]) |> Repo.all

    bottle_changes = from(bc in ApasServer.BottleChange, order_by: [desc: bc.inserted_at], select:  bc.inserted_at) |> Repo.all
